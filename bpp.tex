...
As a side effect it would overwrite \textbf{\$TEMP} variable. As a countermeasure I added a little stack-frame just for this variable. Because the algorithm uses recursion to evaluate chained function calls, I could implement the stack-frame using Java's native stack-frame. This solution was simple to implement, but it has a problem. If one of the member functions has a syntax error in it, the interpreter throws an InvalidSyntaxException and the stack frame will be lost. It is not recommended to use \textbf{\$TEMP} as a variable name. 
...
I struggled on one particular bug in Graphviz-java library related to edge knot points extraction. If dot code from Graphviz contained any edge, that was long enough to have more than 9 anchor points, it would line-break the dot file in the middle of \textbf{pos} attribute. This caused Graphviz-java to parse it incorrectly. Fixing this issue made me thing if using Graphviz-java was worth it in the first place.
