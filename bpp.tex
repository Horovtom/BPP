...
Apart from stak frames, all of these upgrades are a quality-of-life improvements. For example the live preview of generated images would certainly enhance the user experience.
...
Upgrade of the function $f$, described in section \ref{subsec:edge-angle} would improve diagrams of automata with longer edges. This function should be implemented to cz.cvut.fel.horovtom.jasl.graphviz.DotToTex.getCurveAngle function. 

Graphviz does not curve longer edges evenly. Such an edge can be seen on the following picture from state 0 to state 4.
